...
ApplicationBase::ApplicationBase()
    : width(0), height(0), framebufferResized(false), instance(nullptr),
      physicalDevice(nullptr), device(nullptr), graphicsQueue(nullptr),
      presentQueue(nullptr), swapChain(nullptr), pipelineLayout(nullptr),
      renderPass(nullptr), graphicsPipeline(nullptr), commandPool(nullptr),
      commandBuffer(nullptr), imageAvailableSemaphore(nullptr),
      renderFinishedSemaphore(nullptr), inFlightFence(nullptr) {}
...
ApplicationBase::querySwapChainSupport(VkPhysicalDevice device) {
...
bool ApplicationBase::createLogicalDevice() {
    ...
    queueCreateInfo.queueFamilyIndex =
        indices.graphicsFamily
            .value(); // chose the queue which support graphics
...
VkShaderModule
ApplicationBase::createShaderModule(const std::vector<char> &code) {
...
void ApplicationBase::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                                 VkDeviceSize size) {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
